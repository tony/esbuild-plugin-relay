...
test("handles fragments", () => {
    const code = compile_1.compile("./src/test.ts", "graphql`fragment TestFrag on Node { id }`", { artifactDirectory: "./src/__generated__" });
    expect(code).toMatchSnapshot();
});
test("handles queries", () => {
    const code = compile_1.compile("./test/some/nested/folder/file.ts", "graphql`query SomeQuery($id: ID!) { node(id: $id) { id }}`", { artifactDirectory: "./src/__generated__" });
    expect(code).toMatchSnapshot();
});
test("handles mutations", () => {
    const code = compile_1.compile("../another/folder/file.js", "graphql`mutation CreateUser($name: String!) { createUser(name: $name) { id }}`", { artifactDirectory: "./__generated__" });
    expect(code).toMatchSnapshot();
});
test("handles subscriptions", () => {
    const code = compile_1.compile("../another/folder/file.js", "graphql`subscription FeedbackLikeSubscription { likes }`", { artifactDirectory: "./__generated__" });
    expect(code).toMatchSnapshot();
});
test("fail on nameless operation", () => {
    expect(() => {
        compile_1.compile("./src/test.ts", "graphql`{ node { id }}`", {
...
describe("with ESM modules", () => {
  ...
  \`;`;
        expect(compile_1.compile("./src/some/file.js", code, {
  ...
  function SomeComponent() {
    ...
    return (
      <View>
        ...
        expect(compile_1.compile("./src/components/SomeComponent.js", code, {
            suffix: ".js",
...
describe("with enabled development mode", () => {
    test("compares the hash in an ESM module", () => {
        const code = compile_1.compile("src/file.js", `
    /**
     * Copyright (c) Example Inc.
     *
     * This source code is licensed under the MIT license found in the
    ...
    test("compares the hash in an ESM module with a custom condition", () => {
        const code = compile_1.compile("src/nested/dir/component.js", `
  function Component() {
  ...
  \`);
    ...
    test("compares the hash in a CJS module with a custom condition", () => {
        const code = compile_1.compile("build/file.js", `
      fetchQuery<AppQuery>(
        environment,
        graphql\`
